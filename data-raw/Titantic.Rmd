...
Passengers$survival_outcome <- str_detect(TempLines[str_detect(TempLines, "<tr")], "style")[GrabWhich]
```
...
for (i in 1:8){
  ...
  temp$survival_outcome <- str_detect(TempLines[str_detect(TempLines, "<tr")], "style")[1:nrow(temp)]
...
Table$survival_outcome <- ifelse(Table$survival_outcome, "Survived", "Perished")
```
...
table(Table$survival_outcome, Table$Lifeboat)
...
```{r}
# Age
Table$age_character <- Table$Age
table(Table$age_character, useNA = "ifany")
...
```{r}
names(Table) <- str_replace(tolower(names(Table)), " ", "_")
```
...
str(Table)
write.csv(Table, here("data-raw", "DataProducts", "PeopleOnTitanic.csv"), row.names = F)

# rename to final data table name and save for package use
titanic_complete <- Table 
